...
| Tareas        | `GET/POST /tareas-operativas`, `GET/PATCH /tareas-operativas/:id` y `PATCH /tareas-operativas/:id/estado`. Los filtros son `estado`, `responsableId`, `aulaId` y `fecha`. `POST` acepta `aulaId` para una sala o `aulaIds` para crear un grupo: genera una tarea independiente por aula con el mismo `grupoId`, por lo que cada sala conserva estado, responsables e informes propios. Una tarea bloqueante requiere al menos un aula, inicio y fin. |
