...
export class DocentesService {
  ...
  async findAll(query?: string, page?: number, limit?: number) {
    const where = query ? { OR: [{ nombre: { contains: query, mode: 'insensitive' as const } }, { documento: { contains: query, mode: 'insensitive' as const } }, { correo: { contains: query, mode: 'insensitive' as const } }, { proyecto: { contains: query, mode: 'insensitive' as const } }] } : undefined;
    if (!page) return this.prisma.docente.findMany({ where, orderBy: { nombre: 'asc' } });
    const take = Math.min(Math.max(limit ?? 25, 1), 100);
    const [data, total] = await this.prisma.$transaction([this.prisma.docente.findMany({ where, orderBy: { nombre: 'asc' }, skip: (page - 1) * take, take }), this.prisma.docente.count({ where })]);
