...
export class DocentesService {
  ...
  async importarExcel(archivo: { buffer: Buffer; originalname: string } | undefined, usuarioId?: string) {
    ...
    const encabezados = Object.keys(filas[0]).map(normalizar); const faltantes = ['ID', 'NOMBRE'].filter((item) => !encabezados.includes(item));
    ...
    const datos = filas.flatMap((fila, indice) => {
      const documento = valor(fila, 'ID'); const nombre = valor(fila, 'NOMBRE');
      if (!/^\d{3,50}$/.test(documento)) throw new BadRequestException(`Fila ${indice + 2}: el ID debe contener solo números.`);
      if (!nombre || nombre.length > 160) throw new BadRequestException(`Fila ${indice + 2}: el nombre es obligatorio y no puede superar 160 caracteres.`);
