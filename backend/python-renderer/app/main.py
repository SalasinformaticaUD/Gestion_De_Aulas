...
def configure_landscape(sheet_xml: ET.Element) -> None:
    # Fuerza una página horizontal y ajusta el ancho a una sola página para
    # evitar que las columnas de estado queden cortadas en el PDF.
    page_setup = sheet_xml.find(qname(MAIN_NS, "pageSetup"))
    if page_setup is None:
        page_setup = ET.SubElement(sheet_xml, qname(MAIN_NS, "pageSetup"))
    ...
    sheet_properties = sheet_xml.find(qname(MAIN_NS, "sheetPr"))
    if sheet_properties is None:
        sheet_properties = ET.Element(qname(MAIN_NS, "sheetPr"))
        sheet_xml.insert(0, sheet_properties)
    page_setup_properties = sheet_properties.find(qname(MAIN_NS, "pageSetUpPr"))
    if page_setup_properties is None:
        page_setup_properties = ET.SubElement(
            sheet_properties, qname(MAIN_NS, "pageSetUpPr")
        )
...
def rotate_pdf_landscape(content: bytes) -> bytes:
    ...
    for page in source:
        ...
        if width >= height:
            # Algunas salidas de LibreOffice ya traen el contenido horizontal
            # aunque el visor conserve una orientación vertical en metadatos.
            target_width, target_height, rotation = width, height, 0
        else:
            target_width, target_height, rotation = height, width, 90
        landscape = result.new_page(width=target_width, height=target_height)
        landscape.show_pdf_page(landscape.rect, source, page.number, rotate=rotation)
        horizontal_rect = fitz.Rect(0, 0, target_width, target_height)
        landscape.set_mediabox(horizontal_rect)
        landscape.set_cropbox(horizontal_rect)
    rendered = result.tobytes(garbage=4, deflate=True)
    result.close()
    source.close()
    return rendered
...
def render_xlsx(request: RenderRequest):
    ...
    try:
        with tempfile.TemporaryDirectory(prefix="sgoas-render-") as workdir:
            ...
            content = rotate_pdf_landscape(pdf.read_bytes())
