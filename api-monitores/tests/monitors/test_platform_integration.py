...
@pytest.mark.django_db
def test_provision_requires_complete_numeric_identity_fields(api_client, monkeypatch):
    admin = AdminUserFactory()
    api_client.force_authenticate(user=admin)
    ...
    response = api_client.post(
        "/api/v1/monitors/provision/",
        {
            ...
            "codigo_estudiante": "codigo-no-numerico",
            "email": "incompleto@udistrital.edu.co",
            "department": "physics",
            "numero_documento": "documento-no-numerico",
            "proyecto_curricular": "",
            "telefono": "telefono-no-numerico",
        },
        format="json",
    )

    assert response.status_code == 400
    assert {"codigo_estudiante", "numero_documento", "proyecto_curricular", "telefono"} <= set(response.data)
