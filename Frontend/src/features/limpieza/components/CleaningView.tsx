...
export function CleaningView() {
  ...
  return <>
    ...
    <MatrixView matrix={matrix} records={records} loading={loadingMatrix} month={matrixMonth} statusFilter={matrixStatus} suggestions={suggestions} observations={observations} registrationMode={registrationMode} pendingChanges={pendingChanges} onMonthChange={(value) => { setMatrixMonth(value); void loadMatrix(value); }} onStatusChange={setMatrixStatus} onRegisterToday={() => registrationMode ? void acceptPendingChanges() : setRegistrationMode(true)} onSuggestionClick={(roomId) => { const existing = records.find((record) => record.roomId === roomId && record.performedAt.slice(0, 10) === today); setRegistrationMode(true); setMatrixCell({ roomId, date: today, status: existing?.status === "NOVEDAD" ? "novedad" : existing ? "realizado" : "sin-registro", readOnly: false, ...(existing ? { record: existing } : {}) }); }} onCancelRegistration={cancelPendingChanges} onCellClick={(roomId, date, canEdit, status, blockReason) => { const existing = records.find((record) => record.roomId === roomId && record.performedAt.slice(0, 10) === date); setMatrixCell({ roomId, date, status, readOnly: !canEdit, ...(blockReason ? { blockReason } : {}), ...(existing ? { record: existing } : {}) }); }} />
...
function MatrixStatusDialog({ room, date, status, observation, blockReason, onClose }: { room?: Room; date: string; status: "realizado" | "sin-registro" | "novedad"; observation?: string; blockReason?: string; onClose: () => void }) {
  ...
  return <div className={styles.backdrop} role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><section className={styles.dialog} role="dialog" aria-modal="true" aria-labelledby="matrix-status-title"><header><div><span>{blockReason ? "Registro no disponible" : "Consulta de matriz"}</span><h2 id="matrix-status-title">{blockReason ? "Limpieza restringida" : "Estado del aula"}</h2><p>{room?.code ?? "Aula"} · {date}</p></div><button type="button" onClick={onClose} aria-label="Cerrar">×</button></header><div className={styles.statusDialogBody}><strong className={blockReason ? styles.status_blocked : styles[`status_${status}`]}>{blockReason ? "No se puede registrar" : label}</strong><p>{blockReason || observation || (status === "sin-registro" ? "No hay limpieza registrada para este día." : "No hay observaciones adicionales.")}</p><small>{blockReason ? "El aula volverá a estar disponible para registro después de cumplir el intervalo de dos días." : "Para modificar esta celda active “Registrar aseo de hoy” y seleccione una fecha habilitada."}</small><button type="button" className="button-primary" onClick={onClose}>Entendido</button></div></section></div>;
}
...
function CleaningDialog({ rooms, item, initialRoomId, initialDate, onClose, onSave, onRemove }: { rooms: Room[]; item?: CleaningRecord; initialRoomId?: string; initialDate?: string; onClose: () => void; onSave: (payload: { roomIds: string[]; performedAt: string; observation?: string; roomCode: string; status: "realizado" | "novedad" }, item?: CleaningRecord) => void | Promise<void>; onRemove?: () => void }) {
  ...
  const occupiedSuggestions = suggestions.filter((suggestion) => suggestion.enClase && roomIds.includes(suggestion.aula.id));
  ...
  const save = () => { if (!roomIds.length || !room) return; const detalle = observation.trim() || (state === "novedad" ? "Novedad registrada durante la limpieza." : undefined); onSave({ roomIds, roomCode: room.code, performedAt: new Date(performedAt).toISOString(), status: state, observation: detalle }, item); };
  const submit = (event: React.FormEvent) => { event.preventDefault(); if (!item && occupiedSuggestions.length) { setConfirmOccupied(true); return; } save(); };
  return <div className={styles.backdrop} role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><section className={styles.dialog} role="dialog" aria-modal="true" aria-labelledby="cleaning-dialog-title"><header><div><span>Registro operativo</span><h2 id="cleaning-dialog-title">{item ? "Editar limpieza" : "Registrar limpieza"}</h2><p>{initialRoomId ? "Seleccione el resultado de la limpieza para el aula y día elegidos." : item ? "Actualice el registro de limpieza." : "Seleccione una o varias aulas recomendadas para la jornada."}</p></div><button type="button" onClick={onClose} aria-label="Cerrar">×</button></header><form onSubmit={submit}><div className={styles.formGrid}>{initialRoomId ? <label><span>Aula</span><input value={room?.code ?? ""} readOnly /></label> : item ? <label><span>Aula</span><select value={roomIds[0] ?? ""} onChange={(event) => setRoomIds([event.target.value])}>{rooms.map((current) => <option key={current.id} value={current.id}>{current.code}</option>)}</select></label> : <fieldset><legend>Aulas recomendadas</legend>{loadingSuggestions ? <p>Consultando aulas recomendadas…</p> : availableRooms.length ? availableRooms.map((current) => { const enClase = suggestions.some((suggestion) => suggestion.aula.id === current.id && suggestion.enClase); return <label key={current.id}><input type="checkbox" checked={roomIds.includes(current.id)} onChange={(event) => { setConfirmOccupied(false); setRoomIds((selected) => event.target.checked ? [...selected, current.id] : selected.filter((id) => id !== current.id)); }} /> <span>{current.code} · {current.capacity} puestos{enClase ? " · En clase" : ""}</span></label>; }) : <p>No hay aulas que cumplan el criterio de recomendación.</p>}</fieldset>}<label><span>Fecha</span><input type={initialRoomId ? "text" : "datetime-local"} value={initialRoomId ? initialDate ?? "" : performedAt} onChange={(event) => !initialRoomId && setPerformedAt(event.target.value)} readOnly={Boolean(initialRoomId)} required /></label><label><span>Estado</span><select value={state} onChange={(event) => setState(event.target.value as "realizado" | "novedad")}><option value="realizado">Realizado (R)</option><option value="novedad">Novedad (N)</option></select></label><label className={styles.wideField}><span>Observación <small>{state === "novedad" ? "Detalle la novedad" : "Opcional"}</small></span><textarea value={observation} onChange={(event) => setObservation(event.target.value)} rows={5} placeholder={state === "novedad" ? "Describa la novedad encontrada..." : "Observación opcional..."} /></label></div>{confirmOccupied && <div className={styles.occupancyConfirmation} role="alert"><strong>El aula está en clase actualmente</strong><span>Según la información suministrada por Disponibilidad, {occupiedSuggestions.map((suggestion) => suggestion.aula.codigo).join(", ")} {occupiedSuggestions.length === 1 ? "tiene" : "tienen"} una clase en este bloque. ¿Está seguro de registrar la limpieza de todas formas?</span><div><button type="button" className={styles.dialogCancel} onClick={() => setConfirmOccupied(false)}>Volver</button><button type="button" className="button-primary" onClick={() => { setConfirmOccupied(false); save(); }}>Sí, registrar limpieza</button></div></div>}{confirmRemove && <div className={styles.removeConfirmation}><strong>¿Quitar este registro de limpieza?</strong><span>La celda volverá a quedar sin registro.</span><div><button type="button" className={styles.dialogCancel} onClick={() => setConfirmRemove(false)}>Conservar</button><button type="button" className={styles.removeButton} onClick={onRemove}>Quitar registro</button></div></div>}<div className={styles.formSummary}><span>Aulas seleccionadas <strong>{roomIds.length}</strong></span>{room && <span>{room.capacity} puestos</span>}</div><footer>{onRemove && <button type="button" className={styles.removeButton} onClick={() => setConfirmRemove(true)}>Quitar estado</button>}<button type="button" className={styles.dialogCancel} onClick={onClose}>Cancelar</button><button type="submit" className="button-primary" disabled={!roomIds.length || (!item && loadingSuggestions)}>{item ? "Guardar cambios" : "Guardar registro"}</button></footer></form></section></div>;
}
function MatrixView({ matrix, records, loading, month, statusFilter, suggestions, observations, registrationMode, pendingChanges, onMonthChange, onStatusChange, onRegisterToday, onSuggestionClick, onCancelRegistration, onCellClick }: { matrix: CleaningMatrix | null; records: CleaningRecord[]; loading: boolean; month: string; statusFilter: "todas" | "realizado" | "sin-registro" | "novedad"; suggestions: CleaningSuggestion[]; observations: OperationalObservation[]; registrationMode: boolean; pendingChanges: PendingMatrixChange[]; onMonthChange: (value: string) => void; onStatusChange: (value: "todas" | "realizado" | "sin-registro" | "novedad") => void; onRegisterToday: () => void; onSuggestionClick: (roomId: string) => void; onCancelRegistration: () => void; onCellClick: (roomId: string, date: string, canEdit: boolean, status: "realizado" | "sin-registro" | "novedad", blockReason?: string) => void }) {
  ...
  return <div className={styles.matrixWorkspace}>
    <section className={styles.contentCard}>
      ...
      {loading && !matrix ? <p className={styles.emptyTable}>Cargando matriz…</p> : matrix && <div className={styles.matrixScroll}><table className={styles.matrixTable}><thead><tr><th>Aula</th>{matrix.fechas.map((fecha) => <th key={fecha} className={isWeekend(fecha) ? styles.weekendHeader : undefined}>{fecha.slice(8)}<small>{new Intl.DateTimeFormat("es-CO", { weekday: "short" }).format(new Date(`${fecha}T12:00:00`))}</small></th>)}</tr></thead><tbody>{rows.map((aula) => <tr key={aula.id}><th><strong>{aula.codigo}</strong></th>{aula.jornadas.map((jornada) => { const fechasRestringidas = [previousDay(jornada.fecha), daysBefore(jornada.fecha, 2)]; const tieneRegistroReciente = records.some((record) => record.roomId === aula.id && fechasRestringidas.includes(record.performedAt.slice(0, 10))); const esFechaFutura = jornada.fecha > todayDate; const esFechaEditable = jornada.fecha === todayDate || jornada.fecha === previousDay(todayDate); const bloqueadaPorIntervalo = registrationMode && esFechaEditable && !jornada.registros.length && tieneRegistroReciente; const editable = registrationMode && esFechaEditable && !bloqueadaPorIntervalo; const roomObservations = observations.filter((observation) => observation.roomId === aula.id).slice(0, 2); const statusLabel = jornada.estado === "realizado" ? "Realizado" : jornada.estado === "novedad" ? "Con novedad" : "Sin registro"; const blockReason = bloqueadaPorIntervalo ? "No se permite registrar limpieza: esta aula ya fue atendida uno o dos días antes." : registrationMode && esFechaFutura ? "No se permite registrar una limpieza con fecha futura." : undefined; const cellTitle = !registrationMode ? "Consultar estado del aula" : !esFechaEditable ? blockReason ?? "Esta fecha está bloqueada para edición" : blockReason ?? `${aula.codigo} · ${jornada.fecha} · seleccionar Realizado o Novedad`; return <td key={jornada.fecha} className={`${styles.matrixCell} ${isWeekend(jornada.fecha) ? styles.weekendCell : ""}`}><button type="button" className={`${styles[`cell_${jornada.estado}`]} ${jornada.pendienteDeGuardar ? styles.pendingSave : ""}`} title={cellTitle} onClick={() => onCellClick(aula.id, jornada.fecha, editable, jornada.estado, blockReason)}>{jornada.estado === "realizado" ? "R" : jornada.estado === "sin-registro" ? "°" : "N"}</button><span className={styles.cellTooltip}><strong>{aula.codigo} · {jornada.fecha}</strong><span className={styles.cellStatus}>Estado: {statusLabel}</span>{jornada.registros[0]?.observacion && <span>{jornada.registros[0].observacion}</span>}{roomObservations.length > 0 && <><em>Observaciones del aula</em>{roomObservations.map((observation) => <span key={observation.id}>{observation.content}</span>)}</>}</span></td>; })}</tr>)}</tbody></table></div>}{!matrix && !loading && <p className={styles.emptyTable}>No hay información de matriz para el mes seleccionado.</p>}
    </section>
    <aside className={styles.recommendationPanel}>
      <header><strong>Aulas recomendadas</strong><span>{suggestions.length} en prioridad</span></header>
      <p>Priorizadas por el tiempo desde la última limpieza. Las aulas en clase conservan su posición y pueden seleccionarse.</p><div>{suggestions.slice(0, 8).map((suggestion) => <button type="button" key={suggestion.aula.id} title={suggestion.enClase ? "Al guardar se solicitará confirmación porque el aula está en clase." : undefined} onClick={() => onSuggestionClick(suggestion.aula.id)}><span className={styles.recommendationRoomHeader}><b>{suggestion.aula.codigo}</b>{suggestion.enClase && <em>En clase</em>}</span><span>{suggestion.diasSinLimpieza === null ? "Sin historial de limpieza" : `${suggestion.diasSinLimpieza} día(s) desde la última limpieza`}</span></button>)}{!suggestions.length && <small>No hay aulas que cumplan el criterio de prioridad.</small>}</div>{cierreMensual > 0 && <div className={styles.monthClosing}>Cierre mensual: {cierreMensual} aula(s) no registran limpieza durante este mes.</div>}<footer className={styles.registrationActions}><button type="button" className="button-primary" onClick={onRegisterToday}>{registrationMode ? `Aceptar registros (${pendingChanges.length})` : "Registrar aseo de hoy"}</button>{registrationMode && <button type="button" className={styles.dialogCancel} onClick={onCancelRegistration}>Cancelar</button>}</footer>
    </aside>
  </div>;
}
