...
export function CleaningView() {
  ...
  return <>
    ...
    <MatrixView matrix={matrix} records={records} loading={loadingMatrix} month={matrixMonth} statusFilter={matrixStatus} suggestions={suggestions} observations={observations} registrationMode={registrationMode} pendingChanges={pendingChanges} onMonthChange={(value) => { setMatrixMonth(value); void loadMatrix(value); }} onStatusChange={setMatrixStatus} onRegisterToday={() => registrationMode ? void acceptPendingChanges() : setRegistrationMode(true)} onCancelRegistration={cancelPendingChanges} onCellClick={(roomId, date, canEdit, status, blockReason) => { const existing = records.find((record) => record.roomId === roomId && record.performedAt.slice(0, 10) === date); setMatrixCell({ roomId, date, status, readOnly: !canEdit, ...(blockReason ? { blockReason } : {}), ...(existing ? { record: existing } : {}) }); }} />
...
function MatrixView({ matrix, records, loading, month, statusFilter, suggestions, observations, registrationMode, pendingChanges, onMonthChange, onStatusChange, onRegisterToday, onCancelRegistration, onCellClick }: { matrix: CleaningMatrix | null; records: CleaningRecord[]; loading: boolean; month: string; statusFilter: "todas" | "realizado" | "sin-registro" | "novedad"; suggestions: CleaningSuggestion[]; observations: OperationalObservation[]; registrationMode: boolean; pendingChanges: PendingMatrixChange[]; onMonthChange: (value: string) => void; onStatusChange: (value: "todas" | "realizado" | "sin-registro" | "novedad") => void; onRegisterToday: () => void; onCancelRegistration: () => void; onCellClick: (roomId: string, date: string, canEdit: boolean, status: "realizado" | "sin-registro" | "novedad", blockReason?: string) => void }) {
