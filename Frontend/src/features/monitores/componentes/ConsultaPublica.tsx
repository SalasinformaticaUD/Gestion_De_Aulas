...
export function ConsultaPublica() {
  ...
  const consultar = async (evento: FormEvent) => {
    ...
    setError("");
    try {
      setResultado(adaptarConsultaPublica(await servicioMonitores.consultaPublica(codigo.trim())));
    } catch (problema) {
      setResultado(null);
      setError(problema instanceof Error ? problema.message : "No fue posible realizar la consulta.");
    } finally {
      setCargando(false);
    }
  };

  return <div className={estilos.portadaPublica}>
    <header className={estilos.marcaPublica}><UniversityLogo className="public-logo" priority /><div><strong>Gestión de Monitores</strong><span>Consulta institucional de horas</span></div><Link className={estilos.botonSecundario} href="/login?app=monitores">Ingresar al módulo</Link></header>
    <main className={estilos.contenidoPublico}>
      ...
      {resultado && <section className={estilos.resultadoPublico}>
        <section className={estilos.tarjeta}><div className={estilos.detalleMonitor}><div><h2>{resultado.monitor.nombre}</h2><p>Código {resultado.monitor.codigo} · {resultado.monitor.dependencia}</p></div><span className={`${estilos.insignia} ${estilos.exito}`}>Registro activo</span></div></section>
        <ResumenHoras resumen={resultado.resumen} incluirAlertas />
        <TablaSesiones sesiones={resultado.sesiones} />
      </section>}
    </main>
  </div>;
}
