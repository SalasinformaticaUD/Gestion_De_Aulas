...
export function HorariosApiView() {
  ...
  const monitoresActuales = useMemo(() => monitores.filter((item) => item.semester_is_active === true && item.is_active), [monitores]);
  const visibles = useMemo(() => {
    ...
    return rows.filter((item) => {
      ...
      return (!textoBuscado || texto.includes(textoBuscado)) && (estado === "TODOS" || (estado === "ACTIVOS" ? activo : !activo));
    }).sort((a, b) => a.monitor_name.localeCompare(b.monitor_name, "es") || a.weekday - b.weekday || a.start_time.localeCompare(b.start_time));
  }, [rows, monitores, buscar, estado]);
  ...
  async function guardarHorario(evento: FormEvent<HTMLFormElement>) {
    ...
    try {
      if (editando) {
        ...
        const actualizado = await servicioMonitores.actualizarHorario(editando.id, datos);
        setRows((actual) => actual.map((item) => item.id === actualizado.id ? actualizado : item));
