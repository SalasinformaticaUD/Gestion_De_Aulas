  ...
  return <>
    ...
    {!cerrada && <>
      {filas.map((monitor) => {
        ...
        return <tr key={monitor.id}><td><strong>{monitor.nombre}</strong><small>{monitor.codigo}</small></td><td>{(resumen?.horasNormales ?? 0).toFixed(1)}</td><td>{(resumen?.horasExtraAprobadas ?? 0).toFixed(1)}</td><td>{(resumen?.horasExtraPendientes ?? 0).toFixed(1)}</td><td>{(resumen?.horasAnotaciones ?? 0).toFixed(1)}</td><td><strong>{totalHoras.toFixed(1)}</strong></td><td>{Math.max(0, 192 - totalHoras).toFixed(1)}</td><td><Link className={estilos.accionTablaFija} href={`/gestion-monitores/registros/${encodeURIComponent(monitor.id)}`} aria-label={`Ver registros de ${monitor.nombre}`}>Ver registros</Link></td></tr>;
      })}
