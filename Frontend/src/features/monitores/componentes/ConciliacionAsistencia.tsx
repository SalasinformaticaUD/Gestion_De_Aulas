  ...
  return <>
    ...
    <section className={estilos.tarjeta}>
      <div className={estilos.barraHerramientas}>
        ...
        <label className={estilos.campo}><span>Dependencia</span><select value={dependencia} onChange={(event) => { setDependencia(event.target.value); paginacion.reiniciar(); }}><option value="">Todas</option>{[...new Set(recurso.datos.map((item) => item.raw_department))].map((item) => <option key={item}>{item}</option>)}</select></label>
    ...
    <section className={`${estilos.tarjeta} ${estilos.historialConciliacion}`}>
      <header><div><h2>Historial reciente de registros</h2><p>{recursoHistorial.datos.count} registros visibles.</p></div></header>
