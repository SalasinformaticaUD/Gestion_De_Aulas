...
function RequestLoanDialog({ teachers, onClose, onSubmit }: { teachers: DocenteCatalogo[]; onClose: () => void; onSubmit: (payload: { docenteId: string; aulaId: string; inicio: string; fin: string; motivo?: string }) => void | Promise<void> }) {
  const [teacherId, setTeacherId] = useState(teachers[0]?.id ?? ""); const [document, setDocument] = useState(""); const [documentError, setDocumentError] = useState<string | null>(null);
  ...
  return <div className={styles.backdrop} role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><section className={styles.dialog} role="dialog" aria-modal="true" aria-labelledby="loan-dialog-title"><header><div><span>Nueva reserva académica</span><h2 id="loan-dialog-title">Solicitud de préstamo docente</h2><p>Seleccione el docente, el bloque y una de las aulas disponibles.</p></div><button type="button" onClick={onClose} aria-label="Cerrar">×</button></header><form onSubmit={submit}>
     <fieldset className={styles.formSection}><legend><b>1</b> Docente y actividad</legend><div className={styles.formGrid}><label className={styles.wideField}><span>Buscar por cédula</span><div className={styles.documentSearch}><input value={document} onChange={(event) => { setDocument(event.target.value); setDocumentError(null); }} inputMode="numeric" placeholder="Ingrese el número de documento" /><button type="button" onClick={findTeacherByDocument}>Buscar</button></div>{documentError && <small className={styles.documentError}>{documentError}</small>}</label><label className={styles.wideField}><span>Docente responsable</span><select value={teacherId} onChange={(event) => { setTeacherId(event.target.value); const selected = teachers.find((item) => item.id === event.target.value); setDocument(selected?.documento ?? ""); setDocumentError(null); }}><option value="">Seleccione docente</option>{teachers.map((item) => <option key={item.id} value={item.id}>{item.nombre} · CC {item.documento ?? "Sin documento"}</option>)}</select></label><label className={styles.wideField}><span>Motivo de la solicitud <small>Opcional · {reason.length}/500</small></span><textarea value={reason} onChange={(event) => setReason(event.target.value)} maxLength={500} rows={3} placeholder="Ej. Taller complementario de la asignatura..." /></label></div></fieldset>
