...
import { buscarDocentesPorNombre, listarDocentes, type DocenteCatalogo } from "@/features/catalogos/api/catalogosApi";
...
function RequestLoanDialog({ teachers, software, onClose, onSubmit }: { teachers: DocenteCatalogo[]; software: InstalledSoftware[]; onClose: () => void; onSubmit: (payload: { docenteId?: string; docenteNuevoNombre?: string; docenteNuevoDocumento?: string; aulaId: string; softwareId?: string; inicio: string; fin: string; motivo?: string }) => void | Promise<void> }) {
  const [teacherId, setTeacherId] = useState(""); const [teacherQuery, setTeacherQuery] = useState(""); const [teacherOptions, setTeacherOptions] = useState<DocenteCatalogo[]>([]); const [selectedTeacher, setSelectedTeacher] = useState<DocenteCatalogo | null>(null); const [teacherMenuOpen, setTeacherMenuOpen] = useState(false); const [searchingTeacher, setSearchingTeacher] = useState(false); const [teacherSearchError, setTeacherSearchError] = useState<string | null>(null); const [otherTeacher, setOtherTeacher] = useState(false); const [otherName, setOtherName] = useState(""); const [otherDocument, setOtherDocument] = useState(""); const [showTeacherCreate, setShowTeacherCreate] = useState(false);
  ...
  const [softwareId, setSoftwareId] = useState("NINGUNO");
  ...
  const teacher = selectedTeacher ?? teachers.find((item) => item.id === teacherId);
  useEffect(() => { const query = teacherQuery.trim(); if (otherTeacher || query.length < 2 || selectedTeacher) { setTeacherOptions([]); return; } const controller = new AbortController(); const timer = window.setTimeout(async () => { setSearchingTeacher(true); setTeacherSearchError(null); try { const results = await buscarDocentesPorNombre(query, controller.signal); setTeacherOptions(results); if (!results.length) setTeacherMenuOpen(false); } catch (cause) { if (!controller.signal.aborted) { setTeacherOptions([]); setTeacherSearchError(cause instanceof Error ? cause.message : "No fue posible buscar docentes."); } } finally { if (!controller.signal.aborted) setSearchingTeacher(false); } }, 250); return () => { window.clearTimeout(timer); controller.abort(); }; }, [otherTeacher, selectedTeacher, teacherQuery]);
  const chooseTeacher = (item: DocenteCatalogo) => { setSelectedTeacher(item); setTeacherId(item.id); setTeacherQuery(item.nombre); setTeacherOptions([]); setTeacherMenuOpen(false); setTeacherSearchError(null); };
  ...
  return <div className={styles.backdrop} role="presentation" onMouseDown={(event) => event.target === event.currentTarget && onClose()}><section className={styles.dialog} role="dialog" aria-modal="true" aria-labelledby="loan-dialog-title"><header><div><span>Nueva reserva académica</span><h2 id="loan-dialog-title">Solicitud de préstamo docente</h2><p>Seleccione el docente, el bloque y una de las aulas disponibles.</p></div><button type="button" onClick={onClose} aria-label="Cerrar">×</button></header><form onSubmit={submit}>
     <fieldset className={styles.formSection}><legend><b>1</b> Docente y actividad</legend><div className={styles.formGrid}><label className={`${styles.wideField} teacher-search`}><span>Buscar docente por nombre</span><input value={teacherQuery} onFocus={() => setTeacherMenuOpen(true)} onBlur={() => window.setTimeout(() => setTeacherMenuOpen(false), 120)} onKeyDown={(event) => { if (event.key === "Enter" && teacherOptions[0]) { event.preventDefault(); chooseTeacher(teacherOptions[0]); } }} onChange={(event) => { setTeacherQuery(event.target.value); setTeacherId(""); setSelectedTeacher(null); setShowTeacherCreate(false); setTeacherMenuOpen(true); setTeacherSearchError(null); }} placeholder="Escriba el nombre del docente" autoComplete="off" required={!otherTeacher} />{teacherMenuOpen && teacherQuery.trim() && !selectedTeacher && <div className="teacher-options" role="listbox">{searchingTeacher ? <p>Buscando docentes…</p> : teacherSearchError ? <p className="is-error">{teacherSearchError}</p> : teacherOptions.map((item) => <button key={item.id} type="button" role="option" onMouseDown={(event) => event.preventDefault()} onClick={() => chooseTeacher(item)}><strong>{item.nombre}</strong><small>{item.documento ? `Identificación ${item.documento}` : "Sin identificación registrada"}</small></button>)}</div>}{selectedTeacher && <small className={styles.documentError}>Seleccionado: {selectedTeacher.nombre}{selectedTeacher.documento ? ` · CC ${selectedTeacher.documento}` : ""}</small>}</label>{!selectedTeacher && !otherTeacher && <button type="button" className={styles.addTeacherButton} onClick={() => { setTeacherMenuOpen(false); setShowTeacherCreate(true); }}><span>¿No encuentra al docente?</span> Registrar nuevo docente</button>}{otherTeacher && <small className={styles.documentError}>Nuevo docente: {otherName} · CC {otherDocument}</small>}<label className={styles.wideField}><span>Motivo de la solicitud <small>Opcional · {reason.length}/500</small></span><textarea value={reason} onChange={(event) => setReason(event.target.value)} maxLength={500} rows={3} placeholder="Ej. Taller complementario de la asignatura..." /></label></div></fieldset>
    <fieldset className={styles.formSection}><legend><b>2</b> Software y bloque</legend><div className={styles.formGrid}><label className={styles.wideField}><span>Software requerido</span><select value={softwareId} onChange={(event) => setSoftwareId(event.target.value)} required><option value="NINGUNO">Ninguno</option>{software.filter((item) => item.id !== "NINGUNO").map((item) => <option key={item.id} value={item.id}>{item.name}{item.version ? ` · ${item.version}` : ""}</option>)}</select></label><label><span>Fecha</span><input type="date" min={today()} value={date} onChange={(event) => setDate(event.target.value)} required /></label><label><span>Horario</span><select value={startTime} onChange={(event) => setStartTime(event.target.value)} disabled={!availableLoanBlocks.length}>{availableLoanBlocks.map((block) => <option key={block} value={block}>{block}–{String(Number(block.slice(0, 2)) + 2).padStart(2, "0")}:00</option>)}</select></label></div></fieldset>
    ...
    <footer><button type="button" className={styles.dialogCancel} onClick={onClose}>Cancelar</button><button type="submit" className="button-primary" disabled={(!teacher && !otherTeacher) || !selectedRoom || !softwareId}>Crear solicitud →</button></footer>
  </form>{showTeacherCreate && <TeacherCreateDialog initialName={teacherQuery} onClose={() => setShowTeacherCreate(false)} onCreated={(name, document) => { setOtherTeacher(true); setOtherName(name); setOtherDocument(document); setTeacherQuery(name); setShowTeacherCreate(false); }} />}</section></div>;
}

function TeacherCreateDialog({ initialName, onClose, onCreated }: { initialName: string; onClose: () => void; onCreated: (name: string, document: string) => void }) {
  const [name, setName] = useState(initialName);
  const [document, setDocument] = useState("");
  return <div className={styles.backdrop} role="presentation"><section className={styles.dialog} role="dialog" aria-modal="true"><header><div><span>Docente no encontrado</span><h2>Registrar nuevo docente</h2><p>Complete sus datos para usarlo en esta solicitud.</p></div><button type="button" onClick={onClose}>×</button></header><form onSubmit={(event) => { event.preventDefault(); if (name.trim() && /^\d+$/.test(document)) onCreated(name.trim(), document); }}><div className={styles.formGrid}><label className={styles.wideField}><span>Nombre completo</span><input value={name} onChange={(event) => setName(event.target.value)} required autoFocus /></label><label className={styles.wideField}><span>Cédula</span><input value={document} onChange={(event) => setDocument(event.target.value.replace(/\D/g, ""))} inputMode="numeric" required /></label></div><footer><button type="button" className={styles.dialogCancel} onClick={onClose}>Cancelar</button><button type="submit" className="button-primary" disabled={!name.trim() || !document}>Usar nuevo docente</button></footer></form></section></div>;
}
