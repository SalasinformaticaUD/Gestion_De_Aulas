...
import { initialAssignments, initialImports, initialSoftware } from "@/features/software/data/software";
...
export function SoftwareView() {
  ...
  const saveSoftware = (payload: Omit<InstalledSoftware, "id">, id?: string) => {
    ...
    if (id) {
      setSoftware((current) => current.map((item) => item.id === id ? { ...payload, id } : item));
      setNotice({ tone: "success", text: `${payload.name} ${payload.version} fue actualizado.` });
    } else {
      const nextId = `20000000-0000-4000-8000-${String(software.length + 1).padStart(12, "0")}`;
      setSoftware((current) => [...current, { ...payload, id: nextId }].sort((a, b) => a.name.localeCompare(b.name, "es")));
  ...
  const deleteSoftware = (item: InstalledSoftware) => {
  ...
  const assignSoftware = (assignment: SoftwareAssignment) => {
  ...
  const removeAssignment = (assignment: SoftwareAssignment) => {
  ...
  const importRows = (rows: ImportRow[], fileName?: string) => {
    ...
    const nextImportNumber = Math.max(...imports.map((item) => Number(item.id.slice(-4)))) + 1;
  ...
  return <>
    <section className={`page-heading ${styles.heading}`}>
      ...
      <div className={styles.headingActions}><button type="button" className={styles.secondaryButton} onClick={() => setShowAssignment(true)}>Asignar a aula</button><button type="button" className="button-primary" onClick={() => setEditor("new")}>+ Nuevo software</button></div>
    </section>
    ...
    {view === "catalogo" && <CatalogView software={visibleSoftware} assignments={assignments} query={query} roomFilter={roomFilter} onQuery={setQuery} onRoomFilter={setRoomFilter} onEdit={setEditor} onDelete={deleteSoftware} />}
    ...
    {showAssignment && <AssignmentDialog software={software} assignments={assignments} onClose={() => setShowAssignment(false)} onAssign={assignSoftware} />}
...
function CatalogView({ software, assignments, query, roomFilter, onQuery, onRoomFilter, onEdit, onDelete }: { software: InstalledSoftware[]; assignments: SoftwareAssignment[]; query: string; roomFilter: string; onQuery: (value: string) => void; onRoomFilter: (value: string) => void; onEdit: (item: InstalledSoftware) => void; onDelete: (item: InstalledSoftware) => void }) {
...
function RoomsSoftwareView({ software, assignments, selected, matchingRooms, onToggle, onClear, onRemove }: { software: InstalledSoftware[]; assignments: SoftwareAssignment[]; selected: string[]; matchingRooms: typeof rooms; onToggle: (id: string) => void; onClear: () => void; onRemove: (assignment: SoftwareAssignment) => void }) {
  return <section className={styles.roomsLayout}><aside className={styles.requirements}><header><div><span>Búsqueda combinada</span><h2>Aulas por software</h2></div>{selected.length > 0 && <button type="button" onClick={onClear}>Limpiar</button>}</header><p>Seleccione uno o varios programas. Solo aparecerán aulas que tengan instalados todos los seleccionados.</p><div>{software.map((item) => <label key={item.id} className={selected.includes(item.id) ? styles.selectedRequirement : ""}><input type="checkbox" checked={selected.includes(item.id)} onChange={() => onToggle(item.id)} /><span><strong>{item.name}</strong><small>{item.version}</small></span></label>)}</div></aside><div className={styles.roomsResult}><header><div><h2>{selected.length ? "Aulas compatibles" : "Inventario por aula"}</h2><p>{selected.length ? `${matchingRooms.length} aula(s) contienen los ${selected.length} programas seleccionados.` : "Seleccione programas para cruzar requisitos o revise todas las instalaciones."}</p></div><span>{matchingRooms.length} resultados</span></header><div className={styles.roomGrid}>{matchingRooms.map((room) => {
    ...
    return <article key={room.id}><header><div><strong>Aula {room.code}</strong><small>{room.location}</small></div><b>{roomAssignments.length} programas</b></header><div>{roomAssignments.map((assignment) => {
      const item = software.find((softwareItem) => softwareItem.id === assignment.softwareId);
      return item && <span key={assignment.softwareId}><i><strong>{item.name}</strong><small>{item.version} · Instalado {assignment.installedAt}</small></i><button type="button" onClick={() => onRemove(assignment)} aria-label={`Retirar ${item.name} del aula ${room.code}`}>×</button></span>;
...
function SoftwareDialog({ item, onClose, onSave }: { item?: InstalledSoftware; onClose: () => void; onSave: (payload: Omit<InstalledSoftware, "id">, id?: string) => boolean }) {
  const [name, setName] = useState(item?.name ?? "");
  const [version, setVersion] = useState(item?.version ?? "");
  const [description, setDescription] = useState(item?.description ?? "");
  const submit = (event: React.FormEvent) => { event.preventDefault(); onSave({ name: name.trim(), version: version.trim(), description: description.trim() || undefined }, item?.id); };
...
function AssignmentDialog({ software, assignments, onClose, onAssign }: { software: InstalledSoftware[]; assignments: SoftwareAssignment[]; onClose: () => void; onAssign: (assignment: SoftwareAssignment) => boolean }) {
  ...
  const submit = (event: React.FormEvent) => { event.preventDefault(); if (!duplicate) onAssign({ roomId, softwareId, installedAt }); };
  return <DialogShell title="Asignar software a un aula" subtitle="Nueva instalación" description="El aula y el software deben existir previamente en sus catálogos." onClose={onClose}><form onSubmit={submit} className={styles.dialogForm}><div className={styles.formGrid}><label><span>Aula</span><select value={roomId} onChange={(event) => setRoomId(event.target.value)}>{rooms.map((room) => <option key={room.id} value={room.id}>Aula {room.code} · Piso {room.floor}</option>)}</select></label><label><span>Software</span><select value={softwareId} onChange={(event) => setSoftwareId(event.target.value)}>{software.map((item) => <option key={item.id} value={item.id}>{item.name} · {item.version}</option>)}</select></label><label className={styles.wideField}><span>Fecha de instalación <small>Opcional en el backend</small></span><input type="date" value={installedAt} onChange={(event) => setInstalledAt(event.target.value)} /></label></div>{duplicate && <div className={styles.inlineError}>El software ya está asociado con el aula seleccionada.</div>}<footer><button type="button" className={styles.dialogCancel} onClick={onClose}>Cancelar</button><button type="submit" className="button-primary" disabled={duplicate || !softwareId}>Guardar asociación</button></footer></form></DialogShell>;
}

function ImportDialog({ onClose, onImport }: { onClose: () => void; onImport: (rows: ImportRow[], fileName?: string) => void }) {
  const [fileName, setFileName] = useState("");
  const [value, setValue] = useState("402;Docker Desktop;4.43;Contenedores para desarrollo\n405;Node.js;22 LTS;Entorno de ejecución JavaScript");
