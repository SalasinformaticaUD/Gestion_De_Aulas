...
import { actualizarAula, crearAula, importarAulasExcel, listarAulas, type CrearAulaInput } from "@/features/aulas/api/aulasApi";
...
function descargarPlantillaAulas(rooms: Room[]) {
  const encabezados = ["Aula de software", "Capacidad", "Proyecto", "Año", "Marca y modelo", "Caracteristica", "Necesita Renovación"];
      ...
      <section className="rooms-layout" aria-label="Administración de aulas">
        ...
        {selectedRoom ? <section className="room-detail" aria-labelledby="room-title">
          <header className="room-detail-header">
            ...
            <div className="room-actions"><button type="button" className="button-secondary" onClick={() => setEditingRoom(selectedRoom)}>Editar aula</button></div>
          </header>
...
function RoomTabContent({ room, tab }: { room: Room; tab: Tab }) {
  ...
  return <section className="room-tab-content room-information"><InfoCard label="Identificación" value={`Aula ${room.code}`} /><InfoCard label="Capacidad total" value={`${room.capacity} puestos`} /><InfoCard label="Estado actual" value={statusLabels[room.status]} status={room.status} /><InfoCard label="Proyecto" value={room.curriculumProject} /><InfoCard label="Año" value={room.acquisitionYear ? String(room.acquisitionYear) : "Sin información"} /><InfoCard label="Marca y modelo" value={room.brandModel} /><InfoCard label="Característica" value={room.characteristic} /><InfoCard label="Necesita renovación" value={room.renewalNeeded ? "Sí" : "No"} /><InfoCard label="Equipo base" value={room.hardware} wide /></section>;
}
