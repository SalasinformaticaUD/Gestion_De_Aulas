...
export function ProfileView() {
  ...
  return <>
    <section className={`page-heading ${styles.heading}`}><div><span className={styles.kicker}>Cuenta personal</span><h1>Mi perfil</h1><p>Administre su imagen, seguridad y preferencias de visualización.</p></div><span className={styles.accountStatus}><i />Cuenta activa</span></section>
    <div className={styles.layout}>
      ...
      <div className={styles.sections}>
        <section className={styles.profileSection}><header><div><span>01</span><div><h2>Información personal</h2><p>Datos asociados a su cuenta institucional.</p></div></div><b>Solo lectura</b></header><div className={styles.dataGrid}><DataItem label="Nombre completo" value={profile.fullName} /><DataItem label="Correo institucional" value={profile.email} /><DataItem label="Nombre de usuario" value={profile.username} mono /><DataItem label="Cargo" value={profile.role} /></div><aside className={styles.backendNote}><span>i</span><p>Estos datos provienen de <code>GET /auth/me</code>. Los cambios administrativos de identidad se realizan mediante el módulo de Usuarios.</p></aside></section>
