...
export function TasksView() {
  ...
  const requestTransition = (task: OperationalTask, status: TaskStatus) => {
    ...
    if (status === "COMPLETADA" && task.status === "EN_PROCESO") return setCompleteTask(task);
  ...
  return <>
    ...
    {view === "actual" ? <section className={styles.kanban}>{columns.map((column) => { const items = visibleTasks.filter((task) => task.status === column.status); return <section key={column.status} className={`${styles.column} ${styles[`column_${column.status.toLocaleLowerCase()}`]} ${dragTarget === column.status ? styles.dropReady : ""}`} onDragOver={(event) => { event.preventDefault(); setDragTarget(column.status); }} onDragLeave={() => setDragTarget(null)} onDrop={() => dropTask(column.status)}><header><div><i /><span><strong>{column.label}</strong><small>{column.detail}</small></span></div><b>{items.length}</b></header><div className={styles.cardStack}>{items.map((task) => <TaskCard key={task.id} task={task} users={users} currentUserId={currentUser?.id} dragging={draggedId === task.id} onDragStart={() => setDraggedId(task.id)} onDragEnd={() => { setDraggedId(null); setDragTarget(null); }} onAccept={() => setAcceptTask(task)} onCancel={() => setCancelTask(task)} onComplete={() => setCompleteTask(task)} onReport={() => setReportTask(task)} onView={() => setDetailTask(task)} />)}{!items.length && <div className={styles.emptyColumn}>Sin tareas</div>}</div></section>; })}</section> : <HistoryPanel tasks={visibleTasks} status={historyStatus} onStatus={setHistoryStatus} onView={setDetailTask} />}
...
function TaskCard({ task, users, currentUserId, dragging, onDragStart, onDragEnd, onAccept, onCancel, onComplete, onReport, onView }: { task: OperationalTask; users: Usuario[]; currentUserId?: string; dragging: boolean; onDragStart: () => void; onDragEnd: () => void; onAccept: () => void; onCancel: () => void; onComplete: () => void; onReport: () => void; onView: () => void }) {
  ...
  return <article draggable={canMove} className={`${styles.taskCard} ${dragging ? styles.dragging : ""}`} onDragStart={(event) => { if (!canMove) return; event.dataTransfer.effectAllowed = "move"; onDragStart(); }} onDragEnd={onDragEnd}><header><span>{task.code}</span><div>{task.affectsAvailability && <b>Bloquea aula</b>}</div></header><h3>{task.title}</h3>{task.description && <p>{task.description}</p>}<div className={styles.taskMeta}>{task.roomCode && <span className={styles.roomTag}>Aula {task.roomCode}</span>}{pending && <span className={styles.pendingTag}>Con acciones pendientes</span>}</div><div className={styles.responsibleList}><strong>{responsibleNames(task, users)}</strong></div>{task.status === "PENDIENTE" && <div className={styles.taskActions}><button type="button" onClick={onAccept}>Aceptar</button><button type="button" onClick={onCancel}>Cancelar</button></div>}{task.status === "EN_PROCESO" && <div className={styles.taskActions}>{canReport && <button type="button" onClick={onReport}>Informe de seguimiento</button>}{pending && <button type="button" onClick={onAccept}>Aceptar continuación</button>}<button type="button" onClick={onComplete}>Completar</button><button type="button" onClick={onCancel}>Cancelar</button></div>}{["COMPLETADA", "CANCELADA"].includes(task.status) && <div className={styles.taskActions}><button type="button" onClick={onView}>Ver informe y detalles</button></div>}<footer><span className={styles.assignee}><em>{task.responsibles?.length ? `${task.responsibles.length} responsable(s)` : "Sin responsables"}</em></span><span className={styles.closedHint}>{task.status === "COMPLETADA" ? "Trabajo finalizado" : task.status === "CANCELADA" ? "Cierre registrado" : "Arrastre para cambiar estado"}</span></footer></article>;
}

function TaskDialog({ rooms, onClose, onSave }: { rooms: Room[]; onClose: () => void; onSave: (payload: Record<string, unknown>) => Promise<void> }) {
  const [title, setTitle] = useState(""); const [description, setDescription] = useState(""); const [roomId, setRoomId] = useState(""); const [affects, setAffects] = useState(false); const [saving, setSaving] = useState(false);
  return <Modal title="Nueva tarea" subtitle="La tarea se creará pendiente y sin responsable; el primer usuario que la acepte quedará a cargo." onClose={onClose}><form onSubmit={async (event) => { event.preventDefault(); if (affects && !roomId) return; setSaving(true); try { await onSave({ titulo: title.trim(), descripcion: description.trim() || undefined, aulaId: roomId || undefined, afectaDisponibilidad: affects }); } finally { setSaving(false); } }}><div className={styles.formGrid}><label className={styles.wideField}><span>Título</span><input autoFocus required value={title} onChange={(event) => setTitle(event.target.value)} /></label><label className={styles.wideField}><span>Descripción <small>Opcional</small></span><textarea rows={4} value={description} onChange={(event) => setDescription(event.target.value)} /></label><label><span>Aula <small>{affects ? "Obligatoria" : "Opcional"}</small></span><select value={roomId} onChange={(event) => setRoomId(event.target.value)} required={affects}><option value="">Sin aula asociada</option>{rooms.map((room) => <option key={room.id} value={room.id}>{room.code}</option>)}</select></label></div><label className={styles.impactSwitch}><input type="checkbox" checked={affects} onChange={(event) => setAffects(event.target.checked)} /><span><strong>Afecta la disponibilidad del aula</strong><small>La tarea bloqueará el aula mientras esté en proceso.</small></span></label><DialogFooter onClose={onClose} label={saving ? "Guardando…" : "Crear tarea"} disabled={saving || !title.trim() || (affects && !roomId)} /></form></Modal>;
}
