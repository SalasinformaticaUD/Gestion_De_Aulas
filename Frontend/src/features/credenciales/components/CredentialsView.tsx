...
export function CredentialsView() {
  ...
  const canManageAuthorized = obtenerSesion()?.usuario.roles.some((role) => role.trim().toUpperCase() === "ADMINISTRADOR") ?? false;
  ...
  return <>
    ...
    <section className={styles.contentCard}><div className={styles.toolbar}><label className={styles.search}><span>⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar por nombre o descripción" /></label><span className={styles.resultCount}>{visible.length} resultado(s)</span></div><div className="table-wrap"><table className={styles.credentialsTable}><thead><tr><th>Credencial</th><th>Descripción</th><th>Contraseña</th><th>Autorizados</th><th>Estado</th><th>Acciones</th></tr></thead><tbody>{visible.map((item) => <tr key={item.id}><td><strong>{item.nombre}</strong>{item.usuario && <small>{item.usuario}</small>}</td><td>{item.descripcion || <span className={styles.emptyValue}>Sin descripción</span>}</td><td><div className={styles.passwordCell}><code>{revealed[item.id] ?? "••••••••••••"}</code>{canViewSecret && <button type="button" aria-label={`Mantener presionado para ver la contraseña de ${item.nombre}`} title="Mantenga presionado para ver" onPointerDown={() => void revealWhilePressed(item)} onPointerUp={() => hideSecret(item.id)} onPointerCancel={() => hideSecret(item.id)} onPointerLeave={() => hideSecret(item.id)} onContextMenu={(event) => event.preventDefault()}>◉</button>}</div></td><td><span className={styles.accessCount}>{item.accesos.filter((access) => access.puedeVer).length} usuario(s) · {item.rolesAutorizados.length} rol(es)</span></td><td><span className={`${styles.status} ${item.estado === "ACTIVA" ? styles.statusActive : styles.statusInactive}`}><i />{item.estado === "ACTIVA" ? "Activa" : "Inactiva"}</span></td><td><div className={styles.actions}>{canUpdate && <><button onClick={() => { setSelected(item); setModal("change-password"); }}>Cambiar contraseña</button>{canManageAuthorized && <button onClick={() => { setSelected(item); setModal("roles"); }}>Autorizados</button>}<button onClick={() => { setSelected(item); setModal("edit"); }}>Editar</button></>}{canDelete && <button className={styles.dangerAction} onClick={() => { setSelected(item); setModal("delete"); }}>Eliminar</button>}{!canUpdate && !canDelete && <span className={styles.emptyValue}>Sin acciones</span>}</div></td></tr>)}{!visible.length && <tr><td colSpan={6} className={styles.emptyTable}>No hay credenciales registradas.</td></tr>}</tbody></table></div></section>
