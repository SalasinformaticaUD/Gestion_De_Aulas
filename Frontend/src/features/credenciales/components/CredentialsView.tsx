...
import {
  ...
  eliminarCredencial,
  listarCredenciales,
  listarUsuariosParaCredenciales,
...
export function CredentialsView() {
  ...
  const permissions = useMemo(() => new Set((obtenerSesion()?.usuario.permisos ?? []).map((permission) => permission.toUpperCase())), []);
  const canCreate = permissions.has("CREDENCIALES_CREAR");
  const canUpdate = permissions.has("CREDENCIALES_ACTUALIZAR");
  const canDelete = permissions.has("CREDENCIALES_ELIMINAR");
  const canViewSecret = permissions.has("CREDENCIALES_VER_SECRETO");
  const sessionUserId = obtenerSesion()?.usuario.id;
  const sessionRoles = obtenerSesion()?.usuario.roles ?? [];
  const isAdministrator = sessionRoles.some((role) => role.trim().toUpperCase() === "ADMINISTRADOR");
  const canManageAuthorized = (item: CredencialApi) => isAdministrator || item.creadorId === sessionUserId;
  const canEditCredential = (item: CredencialApi) => {
    if (isAdministrator || item.creadorId === sessionUserId) return true;
    if (item.accesos.some((access) => access.usuarioId === sessionUserId && access.puedeEditar)) return true;
    return false;
  };
  const canViewCredential = (item: CredencialApi) => {
    if (isAdministrator || item.creadorId === sessionUserId) return true;
    if (item.accesos.some((access) => access.usuarioId === sessionUserId && access.puedeVer)) return true;
    return false;
  };
  ...
  return <>
    <section className={`page-heading ${styles.heading}`}><div><h1>Credenciales operativas</h1><p>Mantenga presionado el ojo para ver temporalmente una contraseña.</p></div>{canCreate && <button className="button-primary" onClick={() => { setSelected(null); setModal("new"); }}>+ Nueva credencial</button>}</section>
    {notice && <div className={`${styles.notice}${bad ? ` ${styles.noticeError}` : ""}`}>{notice}<button onClick={() => setNotice(null)}>×</button></div>}
    <section className={styles.contentCard}><div className={styles.toolbar}><label className={styles.search}><span>⌕</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Buscar por nombre o descripción" /></label><span className={styles.resultCount}>{visible.length} resultado(s)</span></div><div className="table-wrap"><table className={styles.credentialsTable}><thead><tr><th>Credencial</th><th>Descripción</th><th>Contraseña</th><th>Autorizados</th><th>Estado</th><th>Acciones</th></tr></thead><tbody>{visible.map((item) => <tr key={item.id}><td><strong>{item.nombre}</strong>{item.usuario && <small>{item.usuario}</small>}</td><td>{item.descripcion || <span className={styles.emptyValue}>Sin descripción</span>}</td><td><div className={styles.passwordCell}><code>{revealed[item.id] ?? "••••••••••••"}</code>{canViewSecret && canViewCredential(item) && <button type="button" aria-label={`Mantener presionado para ver la contraseña de ${item.nombre}`} title="Mantenga presionado para ver" onPointerDown={() => void revealWhilePressed(item)} onPointerUp={() => hideSecret(item.id)} onPointerCancel={() => hideSecret(item.id)} onPointerLeave={() => hideSecret(item.id)} onContextMenu={(event) => event.preventDefault()}>◉</button>}</div></td><td><span className={styles.accessCount}>{item.accesos.filter((access) => access.puedeVer).length} usuario(s)</span></td><td><span className={`${styles.status} ${item.estado === "ACTIVA" ? styles.statusActive : styles.statusInactive}`}><i />{item.estado === "ACTIVA" ? "Activa" : "Inactiva"}</span></td><td><div className={styles.actions}>{canUpdate && canEditCredential(item) && <><button onClick={() => { setSelected(item); setModal("change-password"); }}>Cambiar contraseña</button>{canManageAuthorized(item) && <button onClick={() => { setSelected(item); setModal("roles"); }}>Autorizados</button>}<button onClick={() => { setSelected(item); setModal("edit"); }}>Editar</button></>}{canDelete && item.creadorId === sessionUserId && <button className={styles.dangerAction} onClick={() => { setSelected(item); setModal("delete"); }}>Eliminar</button>}{!canEditCredential(item) && !(canDelete && item.creadorId === sessionUserId) && <span className={styles.emptyValue}>Solo visualización</span>}</div></td></tr>)}{!visible.length && <tr><td colSpan={6} className={styles.emptyTable}>No hay credenciales registradas.</td></tr>}</tbody></table></div></section>
    ...
    {modal === "roles" && selected && <RolesForm item={selected} users={users} onLoad={async () => { setUsers(await listarUsuariosParaCredenciales()); }} onClose={() => setModal(null)} onSave={async (userPermissions) => run(() => Promise.all(userPermissions.map((access) => actualizarAccesoCredencial(selected.id, access.usuarioId, access.puedeVer, access.puedeEditar))), "Permisos actualizados.")} />}
  </>;
}
...
type CredentialData = { nombre: string; usuario?: string; secreto?: string; descripcion?: string; estado?: "ACTIVA" | "INACTIVA"; accesos?: Array<{ usuarioId: string; puedeVer: boolean; puedeEditar: boolean }> };
function CredentialForm(props: { item?: CredencialApi; onClose: () => void; onSave: (data: CredentialData) => Promise<void> }) {
  const [name, setName] = useState(props.item?.nombre ?? ""); const [username, setUsername] = useState(props.item?.usuario ?? ""); const [initialSecret, setInitialSecret] = useState(""); const [description, setDescription] = useState(props.item?.descripcion ?? ""); const [status, setStatus] = useState<"ACTIVA" | "INACTIVA">(props.item?.estado ?? "ACTIVA"); const [saving, setSaving] = useState(false); const [users, setUsers] = useState<UsuarioApi[]>([]); const [access, setAccess] = useState<Record<string, Permission>>({});
  const sessionUserId = obtenerSesion()?.usuario.id;
  useEffect(() => { if (!props.item) void listarUsuariosParaCredenciales().then(setUsers); }, [props.item]);
  const toggleUser = (userId: string, checked: boolean) => setAccess((current) => { const next = { ...current }; if (checked) next[userId] = { puedeVer: true, puedeEditar: false }; else delete next[userId]; return next; });
  const setPermission = (userId: string, puedeEditar: boolean) => setAccess((current) => ({ ...current, [userId]: { puedeVer: true, puedeEditar } }));
  return <Dialog title={props.item ? "Editar credencial" : "Nueva credencial"} onClose={props.onClose}><form onSubmit={async (event) => { event.preventDefault(); setSaving(true); try { await props.onSave({ nombre: name.trim(), usuario: username.trim() || undefined, ...(!props.item && { secreto: initialSecret, accesos: Object.entries(access).map(([usuarioId, permission]) => ({ usuarioId, ...permission })) }), descripcion: description.trim() || undefined, estado: status }); } catch { setSaving(false); } }}><div className={styles.formGrid}><label><span>Nombre</span><input value={name} onChange={(event) => setName(event.target.value)} required /></label><label><span>Estado</span><select value={status} onChange={(event) => setStatus(event.target.value as "ACTIVA" | "INACTIVA")}><option value="ACTIVA">Activa</option><option value="INACTIVA">Inactiva</option></select></label><label className={styles.wideField}><span>Usuario de la credencial</span><input value={username} onChange={(event) => setUsername(event.target.value)} placeholder="Usuario o correo de acceso" /></label>{!props.item && <><label className={styles.wideField}><span>Contraseña inicial *</span><input type="password" value={initialSecret} onChange={(event) => setInitialSecret(event.target.value)} required /></label><section className={`${styles.wideField} ${styles.roleChoices}`}><strong>Usuarios autorizados al crear</strong><small>Seleccione cada usuario y el tipo de acceso. Usted, como creador, y el administrador conservan acceso total automáticamente.</small>{users.filter((user) => user.id !== sessionUserId).map((user) => <label className={styles.checkLine} key={user.id}><input type="checkbox" checked={Boolean(access[user.id]?.puedeVer)} onChange={(event) => toggleUser(user.id, event.target.checked)} />{user.nombreCompleto} ({user.nombreUsuario})<select value={access[user.id]?.puedeEditar ? "EDITAR" : "VER"} disabled={!access[user.id]?.puedeVer} onChange={(event) => setPermission(user.id, event.target.value === "EDITAR")}><option value="VER">Solo visualizar</option><option value="EDITAR">Visualizar y editar</option></select></label>)}</section></>}<label className={styles.wideField}><span>Descripción</span><textarea value={description} onChange={(event) => setDescription(event.target.value)} rows={4} /></label></div><footer><button type="button" className={styles.dialogCancel} onClick={props.onClose} disabled={saving}>Cancelar</button><button className="button-primary" disabled={saving}>{saving ? "Guardando…" : "Guardar"}</button></footer></form></Dialog>;
}
...
type Permission = { puedeVer: boolean; puedeEditar: boolean };
function RolesForm(props: { item: CredencialApi; users: UsuarioApi[]; onLoad: () => Promise<void>; onClose: () => void; onSave: (users: Array<{ usuarioId: string } & Permission>) => Promise<void> }) {
  const [userPermissions, setUserPermissions] = useState<Record<string, Permission>>(Object.fromEntries(props.item.accesos.filter((access) => access.usuarioId !== props.item.creadorId).map((access) => [access.usuarioId, { puedeVer: access.puedeVer, puedeEditar: access.puedeEditar }])));
  const sessionUserId = obtenerSesion()?.usuario.id;
  const onLoad = useRef(props.onLoad); useEffect(() => { void onLoad.current(); }, []);
  const permissionControl = (key: string, value: Permission | undefined, set: React.Dispatch<React.SetStateAction<Record<string, Permission>>>) => <span><select value={value?.puedeEditar ? "EDITAR" : "VER"} disabled={!value} onChange={(event) => set((current) => ({ ...current, [key]: { puedeVer: true, puedeEditar: event.target.value === "EDITAR" } }))}><option value="VER">Solo visualizar</option><option value="EDITAR">Visualizar y editar</option></select></span>;
  return <Dialog title="Usuarios autorizados" onClose={props.onClose}><form onSubmit={(event) => { event.preventDefault(); const existingUserIds = props.item.accesos.filter((access) => access.usuarioId !== props.item.creadorId).map((access) => access.usuarioId); void props.onSave([...new Set([...existingUserIds, ...Object.keys(userPermissions)])].map((usuarioId) => ({ usuarioId, ...(userPermissions[usuarioId] ?? { puedeVer: false, puedeEditar: false }) }))); }}><p className={styles.revealPending}><strong>Propietario: {props.item.creador.nombreCompleto}</strong><span>El administrador conserva acceso total. Defina acceso de solo lectura o de edición para cada usuario.</span></p><div className={styles.roleChoices}><strong>Usuarios</strong>{props.users.filter((user) => user.id !== props.item.creadorId && user.id !== sessionUserId).map((user) => <label className={styles.checkLine} key={user.id}><input type="checkbox" checked={Boolean(userPermissions[user.id]?.puedeVer)} onChange={(event) => setUserPermissions((current) => { const next = { ...current }; if (event.target.checked) next[user.id] = { puedeVer: true, puedeEditar: false }; else delete next[user.id]; return next; })} />{user.nombreCompleto} ({user.nombreUsuario}) {permissionControl(user.id, userPermissions[user.id], setUserPermissions)}</label>)}</div><footer><button type="button" className={styles.dialogCancel} onClick={props.onClose}>Cancelar</button><button className="button-primary">Guardar permisos</button></footer></form></Dialog>;
}
