...
export const eliminarCredencial = (id: string, contrasenaUsuario: string) => solicitarAulas(`/credenciales/${id}`, token(), { method: "DELETE", body: JSON.stringify({ contrasenaUsuario }), notificarAutorizacion: false });
export const cambiarContrasenaCredencial = (id: string, data: { contrasenaUsuario: string; secretoActual: string; secretoNuevo: string }) => solicitarAulas(`/credenciales/${id}/secreto`, token(), { method: "POST", body: JSON.stringify(data), notificarAutorizacion: false });
export const verificarAccesoCredenciales = (contrasena: string) => solicitarAulas<{ desbloqueado: boolean; expiraEn: number }>("/credenciales/verificar-acceso", token(), { method: "POST", body: JSON.stringify({ contrasena }), notificarAutorizacion: false });
