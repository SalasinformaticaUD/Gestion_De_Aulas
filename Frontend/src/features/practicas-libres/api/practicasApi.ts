...
const map = (p: ApiPractice): FreePractice => { const requesterType = p.estudiante ? "ESTUDIANTE" : "DOCENTE"; const responsable = p.estudiante ? { id: p.estudiante.id, code: p.estudiante.codigo, name: p.estudiante.nombre, email: p.estudiante.correo ?? undefined } : { id: p.docente?.id ?? "", code: p.docente?.documento ?? "Sin documento", name: p.docente?.nombre ?? "Docente responsable", email: p.docente?.correo ?? undefined }; return { id: p.id, requesterType, status: p.estado, roomId: p.aulaId, roomCode: p.aula.codigo, start: p.inicio, estimatedEnd: p.finEstimada, actualEnd: p.finReal ?? undefined, responsibleType: p.responsableTipo, requestedSoftware: p.softwareSolicitado ?? "Sin especificar", student: { ...responsable, activeFine: false } }; };
