...
export const crearPractica = async (input: { codigoEstudiante: string; nombreEstudiante: string; correoEstudiante?: string; aulaId: string; softwareId: string; softwareSolicitado: string; responsableTipo: FreePractice["responsibleType"]; inicio: string; finEstimada: string }) => map(await solicitarAulas<ApiPractice>("/practicas-libres", auth(), { method: "POST", body: JSON.stringify(input) }));
